...
export type CategorizeResponse = CategorizeResult | CategorizeError;
...
export async function categorizeExpenseCore(
    rawName: string,
    options: CategorizeOptions
): Promise<CategorizeResponse> {
    try {
        const { googleAiApiKey, pixabayApiKey } = options;
...
{"cleanedName": "cleaned name here", "suggestedCategory": "category here", "suggestedSubcategory": "subcategory here", "brandColor": "#123456", "brandAccentColor": "#654321", "brandLogoUrl": "https://logo.clearbit.com/example.com", "imageKeyword": "keyword here", "imageUrl": "https://images.unsplash.com/photo-id", "confidence": "high"}`;
        ...
        const baseKeyword = rawKeyword.trim() || parsed.cleanedName || normalizedSubcategory || normalizedCategory || rawName;
        ...
        if (!normalizedLogo && baseKeyword) {
            const pixabayImage = await fetchPixabayImage(baseKeyword, normalizedCategory, normalizedSubcategory, pixabayApiKey);
            if (pixabayImage) {
        ...
        const normalizedImage = candidateImage ?? pixabayImageNormalized ?? normalizedLogo ?? fallbackAvatar;
